...
def test_guest_creation():
    """Test case for creating a guest account with exception handling."""

    try:
        # Request user input for guest details
        name = input("Enter Guest Name: ").strip()  # Get guest name, remove extra spaces
        if not name:  # Ensure name is not empty
            raise ValueError("Guest name cannot be empty.")

        contact_info = input("Enter Contact Info (Phone/Email): ").strip()  # Get contact info
        if not contact_info:  # Ensure contact info is provided
            raise ValueError("Contact information cannot be empty.")

        guest_id = input("Enter Guest ID (Numeric): ").strip()  # Get guest ID
        if not guest_id.isdigit():  # Ensure guest ID is a number
            raise ValueError("Guest ID must be a numeric value.")
        guest_id = int(guest_id)  # Convert ID to an integer

        # Create a new Guest object
        guest = Guest(name, contact_info, guest_id)

        # Display success message
        print("\n✅ Guest account created successfully!")
        print(guest)  # Print guest details using __str__()
